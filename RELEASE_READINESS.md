...
| RC-09 | Chrome reports AbortError/InvalidStateError when rapid navigation intentionally cancels a native CSS View Transition | Browser release test | Informational browser event | Remote CI passed navigation/fallback state but logged the two native cancellation forms | None | Record only the exact “Transition was skipped” and “aborted because of invalid state” messages separately; continue failing every other exception | Test false positive only | Native navigation already settled successfully | Resolved; Linux CI passes on the implementation/test head |
